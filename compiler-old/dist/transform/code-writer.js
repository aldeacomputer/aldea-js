import { FieldNode } from './nodes.js';
const AS_INT_TYPES = [
    'i8',
    'i16',
    'i32',
 ...
 */
export function writeClassWrapper(obj, members) {
    return `
  class ${obj.name} {
    ${members.join('\n')}
  }
 ...
 */
export function writeDeserializeStaticMethod(obj) {
    return `
 ...
 */
export function writeSerializeInstanceMethod(obj) {
 ...
 */
export function writeExportMethod(method, obj) {
 ...
 */
export function writeExportSchemaMethod(obj) {
    const mapType = {
        name: 'Map',
 ...
 */
export function writeSchema(fields, obj, chunks = ['const schema = new Map<string, string>()']) {
    return fields.reduce((chunks, field) => {
 ...
 */
export function writeExportDeserializeMethod(obj) {
    return `
 ...
 */
export function writeExportSerializeMethod(obj) {
 ...
 * Returns an AssemblyScript string.
 */
export function writeArgReader(fields, obj, chunks = ['const args = new CborReader(argBuf)']) {
    if (!Array.isArray(fields)) {
        fields = [fields];
    }
    if (!fields.length && chunks.length > 1) {
        chunks.join('\n');
    }
    return fields.reduce((chunks, field, i) => {
 ...
 * Returns an AssemblyScript string.
 */
export function writeArgWriter(fields, obj, chunks = ['const args = new CborWriter()']) {
    if (!Array.isArray(fields)) {
        fields = [fields];
    }
    if (!fields.length && chunks.length > 1) {
 ...
 */
export function writePtrWriter(fields, obj, chunks = [`const ptr = __new(offsetof<${obj.name}>(), idof<${obj.name}>())`]) {
    return fields.reduce((chunks, field, i) => {
        const type = AS_INT_TYPES.includes(field.type.name) ? field.type.name : 'usize';
        const val = AS_INT_TYPES.includes(field.type.name) ? `a${i}` : `changetype<usize>(a${i})`;
 ...
 * Returns an AssemblyScript string.
 */
export function writeReturnWriter(fields, obj) {
    if (!Array.isArray(fields)) {
        fields = [fields];
    }
    ...
    const chunks = fields.reduce((chunks, field) => {
 ...
 */
export function writeMethodCall(method, obj) {
    const prefix = method.rType.name === 'void' ? '' : 'const ctx = ';
    const args = method.args.map((_a, i) => `a${i}`).join(', ');
 ...
 * strings. Returns an AssemblyScript string.
 */
export function writeProxyClassWrapper(obj, members) {
    return `
  class ${obj.name} {
    origin: Uint8Array;
    ${members.join('\n')}
 ...
 */
export function writeProxyGetter(field, obj) {
 ...
 * - Decodes and returns the correct type
 */
export function writeProxyMethod(method, obj) {
    const origin = obj.decorators.find(d => d.name === 'jig')?.args[0];
 ...
 * decoded value.
 */
export function writeProxyReturn(field, obj, isConstructor = false) {
 ...
 * Returns an AssemblyScript string.
 */
function decodeMethod(field, obj) {
    const isRef = field.type.name === obj.name;
    if (isRef) {
        return `decodeRef<${field.type.name}>()`;
    }
    const externalClassNames = obj.ctx.externalClasses.map(n => n.name);
 ...
 * Returns an AssemblyScript string.
 */
function encodeMethod(field, obj) {
    const isRef = field.type.name === obj.name;
    if (isRef) {
