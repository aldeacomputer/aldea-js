...
class TestVisitor implements AbiVisitor {
  ...
  visitSmallNumber(fieldNode: FieldNode): void {
...
describe('AbiTraveler', () => {
  ...
  describe('when the class has a single small number field', () => {
    function checkScenario (typeName: string) {
      ...
      traveler.visitClass('Foo', testVisitor)
      expect(testVisitor.allFields).to.have.length(1)
      expect(testVisitor.numberFields).to.have.length(1)
      expect(testVisitor.numberFields[0]).to.have.eql(
  ...
  describe('when the class has a single big integer field', function () {
    function checkScenario (typeName: string) {
      const abi = new TestAbiBuilder()
        .addClass('Foo', cls => {
          cls.withField(FieldKind.PUBLIC, 'bar', { name: typeName, args: [] })
