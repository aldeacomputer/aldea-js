...
//       const wasm = exec.importModule(pkgId).asContainer
...
//       const exec = emptyExec()
//       const wasm = exec.importModule(moduleId).asContainer
...
//
//       const exec = emptyExec()
//       const wasm = exec.importModule(flockId).asContainer
//       const jig = exec.instantiateByClassName(wasm, 'Flock', []).asJig()
...
//       const wasm = await exec.deployPackage([fileName],  sources).then(stmt => stmt.asContainer)
//       const jig = exec.instantiateByClassName(wasm, 'Foo', []).asJig()
//       exec.lockJigToUser(jig, userAddr)
//       exec.markAsFunded()
//       const ret = exec.finalize()
...
//
//       expect(result.asContainer.id).to.eql(packageId)
//     })
//   })
// })
