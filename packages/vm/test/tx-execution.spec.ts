...
import {Pointer, PrivKey, ref, Tx, instructions} from "@aldea/sdk-js";
...
describe('execute txs', () => {
  ...
  it('creates instances from imported modules', () => {
    ...
    expect(output.classPtr.idBuf).to.eql(modIdFor('flock'))
    expect(output.classPtr.idx).to.eql(0)
  })
  ...
  it('sends arguments to constructor properly.', () => {
    ...
    const parsed = result.outputs[0].props
  ...
  //   const result = exec.finalize()
  //
  //   const parsed = JigState.fromOutput(result.outputs[0]).parsedState()
  ...
  //
  //   const parsed = result.outputs[0].props
  //   expect(parsed.size).to.eql(7) // growMany effect
  // })
  //
