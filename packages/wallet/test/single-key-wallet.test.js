...
import { Aldea, LockType, Output, PrivKey } from '@aldea/sdk'
import { Memory } from 'lowdb'
import { LowDbStorage, SingleKeyWallet } from '../dist/index.js'
...
test.beforeEach(t => {
  ...
  const wallet = new SingleKeyWallet(PrivKey.fromRandom(), storage, t.context.aldea)
  t.context.wallet = wallet
})

test('getNextAddress() returns same address', async t => {
  const addr1 = await t.context.wallet.getNextAddress()
  const addr2 = await t.context.wallet.getNextAddress()
  t.deepEqual(addr1, addr2)
})
