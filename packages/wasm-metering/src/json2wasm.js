...
const LANGUAGE_TYPES = (json2wasm.LANGUAGE_TYPES = {
  i32: 0x7f,
  i64: 0x7e,
  f32: 0x7d,
  f64: 0x7c,
...
const OPCODES = (json2wasm.OPCODES = {
  unreachable: 0x0,
  nop: 0x1,
  block: 0x2,
  loop: 0x3,
  ...
  'i32.trunc_sat_f32_u': [0xfc, 1],
  'i32.trunc_sat_f64_s': [0xfc, 2],
  'i32.trunc_sat_f64_u': [0xfc, 3],
  'i64.trunc_sat_f32_s': [0xfc, 4],
  'i64.trunc_sat_f32_u': [0xfc, 5],
  'i64.trunc_sat_f64_s': [0xfc, 6],
  'i64.trunc_sat_f64_u': [0xfc, 7]
})

json2wasm.typeGenerators = {
  function: (json, stream) => {
    leb.unsigned.write(json, stream)
  },
  table: (json, stream) => {
    stream.write([LANGUAGE_TYPES[json.elementType]])
    json2wasm.typeGenerators.memory(json.limits, stream)
  },
  /**
  ...
  initExpr: (json, stream) => {
    json2wasm.generateOp(json, stream)
    json2wasm.generateOp({ name: 'end', type: 'void' }, stream)
  }
}

json2wasm.immediataryGenerators = {
...
json2wasm.generateSection = function (json, stream = new Stream()) {
...
json2wasm.generate = (json, stream = new Stream()) => {
  ...
  for (let item of rest) {
    json2wasm.generateSection(item, stream)
  }
...
json2wasm.generateOp = (json, stream = new Stream()) => {
  ...
  const data = OPCODES[name]
  if (Array.isArray(data)) {
    stream.write([data[0]])
    leb.unsigned.write(data[1], stream)
  } else {
    stream.write([data])
  }
  ...
  if (immediates) {
    json2wasm.immediataryGenerators[immediates](json.immediates, stream)
  }
  return stream
}
